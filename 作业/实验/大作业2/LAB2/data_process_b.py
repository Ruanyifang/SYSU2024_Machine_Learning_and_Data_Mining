...
# 保存扩展后的词汇表和映射
with open('emotion_vocab.json', 'w', encoding='utf-8') as f:
    json.dump(vocab, f, ensure_ascii=False)

with open('emotion_id2char.json', 'w', encoding='utf-8') as f:
...
def chinese_tokenizer(text):
    return list(jieba.cut(text))  # 使用jieba分词

train_data['tokenized_review'] = train_data['review'].apply(chinese_tokenizer)

# 将词汇表中的词映射为唯一ID，未知词汇用 <unk> 表示
def text_to_sequence(tokens):
    return [vocab.get(token, vocab.get('<unk>')) for token in tokens]

train_data['sequences'] = train_data['tokenized_review'].apply(text_to_sequence)
...
def pad_sequence(seq, max_len=MAX_LEN):
    if len(seq) > max_len:
        ...
        return seq + [0] * (max_len - len(seq))  # 填充

train_data['padded_sequences'] = train_data['sequences'].apply(lambda x: pad_sequence(x, MAX_LEN))

# 将训练集数据转为NumPy数组
X_train = np.array(train_data['padded_sequences'].tolist())
y_train = train_data['label'].values

# 测试集数据处理（与训练集相同逻辑）
test_data['tokenized_review'] = test_data['review'].apply(chinese_tokenizer)
test_data['sequences'] = test_data['tokenized_review'].apply(text_to_sequence)
test_data['padded_sequences'] = test_data['sequences'].apply(lambda x: pad_sequence(x, MAX_LEN))
X_test = np.array(test_data['padded_sequences'].tolist())
